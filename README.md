...
**Status:** Step 5 / 10 — Voice session backend live: `/api/voice/session` mints ephemeral OpenAI Realtime tokens (GA `gpt-realtime`, voice `marin`) with the full system prompt + 35 tools attached.
...
npm run test:voice       # system prompt + Realtime session config (42 assertions)
npm test                 # typecheck + all five suites (217 assertions total)
```
...
| 5 | Voice session backend + v0 system prompt | ✓ |
